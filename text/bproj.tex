...
\MapleOutput{CI := [{CI1 = -\frac{gm *(\frac{R}{R1+p*L1+1/(p*C1)}-1)* (R1+p*L1+\frac{1}{(p*C1)})}{(R*p)}, CI2 =\frac{ gm*(C1*L1*R1*p^2-L1^2*p^2+R1)}{(L1^2*p^2*s)}, }}
\MapleOutput{CI3 = \frac{gm*(C3*L3*p^2-C3*R1*p+1)}{(C3*R1*p*s)}, }
