...
\section{Úvod}
\noindent Cílem práce je navrhout zapojení analogového filtru typu pásmová propust 4. řádu s OTA za použití knihovny Syntfil, simulace v Multisimu.
\section{Analogové filtry}
Filtry jsou určeny k potlačení nebo zvýraznění určité části kmitočtového spektra signálu. Jsou to obvody s kmitočtově závislou přenosovou funkcí (pro napěťový přenos $H_s(j \omega) = U_{out}(j \omega)/U_{in}(j \omega)$). Základní rozdělení je na dolní propust (\textit{low-pass} - LP), horní propust(\textit{high-pass} - HP), pásmovou propust (\textit{band-pass} - BP) a pásmovou zádrž (\textit{band-stop} - BS). \\
\\
...
\end{figure}
\noindent Mezní kmitočet a činitel jakosti tohoto obvodu byly spočítány jako 
\begin{align}
\omega _c &= \sqrt{\frac{1}{LC}}\\
Q &= \frac{L\sqrt{\frac{C}{L}}}{RC}.
\end{align}
Zapojení obvodu v Multisimu ilustruje Obrázek 14.
\begin{figure}[H]
\centering
...
\end{figure}
\noindent Lze použít i zapojení s pralelně řazeným uzemněným kapacitorem a odporem a indukčností. NA výstupu 2. OTA byl obdržen filtr typu PP. Na výstupu 3. OTA pak DP 2. řádu.
\begin{figure}[H]
\centering
...
\caption{Amplitudová a fázová charakteristika dolní propusti 2. řádu, pásmové propusti}
\end{figure}
...
\caption{Schéma zapojení dolní propusti 4. řádu}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{lrcx2ampl.png}
\caption{Amplitudová a fázová charakteristika dolní propusti 4. řádu}
\end{figure}
\begin{figure}[H]
\centering
...
\end{align}
\noindent Nejprve byla zapojením dvou CR článků obdržena horní propust. Následně byla sériovým zapojením dolní a horní propusti 2. řádu obdržena pásmová propust 2. řádu.
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{1606.png}
\caption{Schéma zapojení horní propusti 2. řádu (2x CR článek)}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{16063.png}
\caption{Amplitudová a fázová charakteristika horní propusti 2. řádu (2x CR článek)}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{lrcbandpass.png}
\caption{Schéma zapojení pásmové propusti 2. řádu (HP - 2x CR článek)} 
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{lrcbandpassampl.png}
\caption{Amplitudová a fázová charakteristika pásmové propusti 2. řádu (HP - 2x CR článek)}
\end{figure}
\noindent Nebo lze zapojit odpor (Obrázek 10), kapacitor a uzemněnou indukčnost (Obrázek 12).
\begin{figure}[H]
\centering
...
\noindent Zapojením dolní propusti 4. řádu a následným zapojením dvou horních propustí 2. řádu byla obdržena pásmová propust 4.řádu. První průběh ukazuje realizaci s horní propustí CR články, druhý s RLC obvodem. Byl obdržen filtr 4. řádu s poklesem -80 dB/dek. Průběh v okolí 0 dB není zcela hladký, protože hodnoty prvků v obvodu nebyly zvoleny přesně.
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{lrcbandpass4.png}
\caption{Schéma zapojení pásmové propusti 4. řádu (HP - 2x CR článek)}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{lrcbandpass4ampl.png}
\caption{Amplitudová a fázová charakteristika pásmové propusti 4. řádu (HP - 2x CR článek)}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[scale=0.5]{bp4sch.png}
\caption{Schéma zapojení pásmové propusti 4. řádu}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{bp42.png}
\caption{Amplitudová a fázová charakteristika pásmové propusti 4. řádu}
\end{figure}
...
\MapleOutput{\Delta{fp} := 130000 Hz}
\MapleOutput{\Delta{fs} := 300000 Hz}
\MapleOutput{ap := 20 dB}
\MapleOutput{as := 80  dB,}
\noindent kde $fm$ značí geometrický střed propustného pásma [Hz], $\Delta{fp}$ šířku propustného pásma [Hz],
$\Delta{fs} $ šířku nepropustného pásma [Hz], $ap$ maximální útlum v propustném pásmu [dB], $as$ minimální útlum v nepropustném pásmu [dB].
...
\end{figure}
\noindent Vyčíslením v $168077\cdot 2 \pi$ Hz byl obdržen útlum 32 dB.\\
\\
...
\MapleOutput{R1=eval(elems\_BPQ)[R1], Rz=eval(elems\_BPQ)[R2];}
...
\MapleOutput{L5 = 3.89179 *10^{-7}, R1 = 1, Rz = 1.004}
...
\end{align}
\noindent Kapacity vyšly odnormovány již se Syntfilu $C1 = 118.426$ nF, $C2 = 2.04513 \  \mu$F, $C3 = 3.88609 \ \mu$F, $C4 = 173.599$ nF, $C5 = 10.1698 \  \mu$F.
\section{Simulace s výsledky ze Syntfilu}
\noindent Vypočtené prvky byly dosazeny do schématu v Multisimu. Bylo použito zapojení se vstupním odporem $R_0$ řazeným paralelně ke zdroji (vhodnější pro funkční simulaci). Napětí na zdroji poté bude $V_0/R_0$.
\begin{figure}[H]
\centering
...
U_2 &= \frac{1}{\frac{1}{pL_2} + pC_2}(I_1 - I_{L3} - I_{L4} - pC_4 v_{L4})\\
...
H & = \frac{gm}{sc + gm}
\end{align}
a porovnáním s předchozími přenosy $H_1,  \ldots, H_5$ byly obdrženy následující hodnoty kapacit
\MapleOutput{CI := [{CI1 = -\frac{gm *(\frac{R}{R1+p*L1+1/(p*C1)}-1)* (R1+p*L1+\frac{1}{(p*C1)})}{(R*p)}, CI2 =\frac{ gm*(C2*L2*R1*p^2-L2*p+R1)}{(L2*p^2)}, }}
\MapleOutput{CI3 = \frac{gm*(C3*L3*p^2-C3*R1*p+1)}{(C3*R1*p^2)}, }
\MapleOutput{CI4 = \frac{gm*(C4*L4*R1*p^2-L4*p+R1)}{(L4*p^2)}, CI5 = \frac{gm*(C5*L5*R1*Rz*p^2+L5*R1*p-L5*Rz*p+R1*Rz)}{(L5*Rz*p^2)}]}
\noindent Zohledněním paralelního zapojení kondenzátoru C2, C4 a C5 k indukčnosti bylo dosazeno za původní hodnoty dosazeno $Cg=C \cdot gm \cdot R$.
...
\\
Cíl práce byl částečně splněn. Bude třeba převést správně LC strukturu na zapojení s OTA, opravit následnou simulaci v Multisimu a analyzovat výslednou strukturu pomocí Pracanu. Dalším krokem bude pak praktická realizace a odzkoušení navrhnutého obvodu.
